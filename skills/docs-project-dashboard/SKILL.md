---
name: docs-project-dashboard
description: Backfill a generated HTML project-management dashboard into repositories that already use the docs-system-scaffold `PROJECTS/` lifecycle structure. Use when the user wants a docs-native portfolio view, local PM dashboard, project index, or at-a-glance status board showing active, in-review, blocked, backlog, stale, and completed planning docs.
---
...
Use this skill to add a generated `PROJECTS/dashboard.html` file plus a reusable repo-side generator script so the dashboard can be regenerated after project-doc changes.

Assume the target repository already follows the docs-system-scaffold project layout:
...
If the repo does not already have a `PROJECTS/` tree, stop and use `$docs-system-scaffold` instead.
