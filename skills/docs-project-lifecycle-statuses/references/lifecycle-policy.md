...
Use this policy when a repo already has product-operating-system-scaffold governance and project tracking under `docs/PROJECTS/` or `DOCS/PROJECTS/`.
Use this skill only for repositories scaffolded before `product-operating-system-scaffold` included `in-review` and `blocked` by default.
